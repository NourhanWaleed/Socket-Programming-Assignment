    ...
    while connected:
        ...
        if message.split()[0] == 'GET' :
            ...
            try:
                ...
                conn.send(bytes("HTTP/1.0 200 OK\r\n","UTF-8"))     #send the Http header which is formatted as 200 OK
               
                
            except: # if file not found
               conn.send(bytes("HTTP/1.0 404 Not Found\r\n","UTF-8"))     #send the Http header which is formatted as 404 Not Found
               
               
        elif message.split()[0] == POST:
            try:
                outputdata = message.split()[1] 
                newpath = shutil.copy(outputdata,"E:\esraa\Computer Networks\prog.Ass\ServerFolder")    #copy the required file into ServerFolder (simulating sending it to server)
                conn.send(bytes("HTTP/1.0 200 OK\r\n","UTF-8"))     
        ...
        conn.send(bytes(outputdata,"UTF-8"))     #Send the content of the requested file to the client
        conn.send("\r\n".encode())
    conn.close()
